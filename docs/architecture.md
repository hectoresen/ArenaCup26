...
- `achievements` — catálogo de **28 logros en 6 tiers** (común, poco común, épico, legendario, mítico, GOAT). Evaluación al cierre oficial de partido (no con provisionales) + gate `ACHIEVEMENTS_MIN_FINISHED_MATCHES` para los de rendimiento. `team-spirit` (común) está en `GATE_BYPASS` — se desbloquea al instante sin esperar al gate. Los tres logros `division-*` (epic/legendary/mythic, 2026-06-17) están ligados a los cortes 30/20/10 del leaderboard. Catálogo formal en `docs/achievements.md`.
