...
- `achievements` — catálogo de **28 logros en 6 tiers** (común, poco común, épico, legendario, mítico, GOAT). Evaluación al cierre oficial de partido (no con provisionales) + gate `ACHIEVEMENTS_MIN_FINISHED_MATCHES` para los de rendimiento. `team-spirit` (común) está en `GATE_BYPASS` — se desbloquea al instante sin esperar al gate. Los tres logros `division-*` (epic/legendary/mythic, 2026-06-17) están ligados a los cortes 30/20/10 del leaderboard — sistema documentado end-to-end en `docs/divisions.md`. Catálogo formal en `docs/achievements.md`.
...
- `bots` — **usuarios sintéticos** (`add-bot-users`, propuesta 2026-05-19). 27 filas en `users` con `is_bot=true` que predicen fase de grupos aleatoriamente para poblar el cold-start del Mundial. Reusan toda la infra (scoring, ranking, achievements). NO entran en grupos, NO inician sesión, NO reciben push. Flag NUNCA expuesto en API. Ver `docs/bots.md`.
