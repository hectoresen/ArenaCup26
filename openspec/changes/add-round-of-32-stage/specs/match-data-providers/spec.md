...
- **Given** los strings nativos: `"Group A - 1"`, `"Round of 32"`, `"Last 32"`, `"1/16 Finals"`, `"Round of 16"`, `"1/8 Finals"`, `"Quarter-finals"`, `"Semi-finals"`, `"3rd Place Final"`, `"Final"`
- **When** se pasan por `parseStage`
- **Then** mapean a `"group"`, `"round-of-32"`, `"round-of-32"`, `"round-of-32"`, `"round-of-16"`, `"round-of-16"`, `"quarter"`, `"semi"`, `"third-place"`, `"final"` respectivamente.

### Scenario: Round of 32 chequeado antes que Round of 16

- **Given** los dos checks coexisten en `parseStage`
- **When** se recibe `"Round of 32"`
- **Then** el primer match (`round-of-32`) se aplica y nunca se evalúa el patrón menos específico de `"round of 16"`. El orden de los `includes()` en el código garantiza precedencia.
...
### Scenario: Round-of-32 pasa por el adapter como cualquier knockout

- **Given** un `ProviderMatch` con `stage: "round-of-32"`, `status: "finished"`, `scoreAt90: {home:1, away:1}`, `scoreAtExtra: {home:2, away:1}`, `penaltyWinner: null`
- **When** se adapta
- **Then** el `MatchOutcome` resultante hereda `stage = "round-of-32"` y los scores tal cual. No requiere ramas nuevas en el adapter.
...
### Scenario: Predicción simple correcta sobre un round-of-32

- **Given** un fixture sintético de `stage: "round-of-32"`, `scoreAt90: {home:1, away:1}`, `scoreAtExtra: {home:2, away:1}`, `penaltyWinner: null`
- **When** se aplica una predicción `simple` con `predictedWinner: "home"`
- **Then** el engine devuelve `points: 10`, `kind: "simple"`, idéntico tratamiento a cualquier otra eliminatoria.
